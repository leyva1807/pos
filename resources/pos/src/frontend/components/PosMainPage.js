...
const PosMainPage = ( props ) => {
        ...
        return (
            <div className='d-none'>
                ...
                <PrintRegisterDetailsData ref={registerDetailsRef} allConfigData={allConfigData} frontSetting={frontSetting} posAllTodaySaleOverAllReport={posAllTodaySaleOverAllReport}
                    updateproducts={paymentPrint} closeRegisterDetails={closeRegisterDetails} />
            </div>
        )
    };
        ...
        return (
            <div className='d-none'>
                <PaymentSlipModal printPaymentReceiptPdf={printPaymentReceiptPdf}
                    ...
                    paymentType={paymentValue.pyment_type.label} paymentTypeDefaultValue={paymentTypeDefaultValue}
                />
            </div>
        )
    };
    ...
    const handleCloseRegisterDetails = ( data ) => {
        if ( data.cash_in_hand_while_closing.toString().trim()?.length === 0 ) {
            dispatch( addToast(
                { text: getFormattedMessage( "pos.cclose-register.enter-total-cash.message" ), type: toastType.ERROR } ) );
        } else {
    ...
    return (
        <Container className='pos-screen px-3' fluid>
                            ...
                            <Table className="mb-0">
                                ...
                                <tbody className='border-0'>
                                    {updateproducts && updateproducts.length ? updateproducts.map( ( updateProduct, index ) => {
                                        return <ProductCartList singleProduct={updateProduct} key={index + 1} index={index}
                ...
                <Col lg={7} xxl={8} xs={6} className='ps-lg-0 pos-right-scs'>
                    <div className='right-content mb-3'>
                        <div className="d-sm-flex align-items-center flex-xxl-nowrap flex-wrap">
                            ...
                            />
                            <HeaderAllButton holdListData={holdListData} goToHoldScreen={onClickHoldModel} goToDetailScreen={onClickDetailsModel} onClickFullScreen={onClickFullScreen} opneCalculator={opneCalculator} setOpneCalculator={setOpneCalculator} handleClickCloseRegister={handleClickCloseRegister} />
                        </div>
            ...
            <PosCloseRegisterDetailsModel showCloseDetailsModal={showCloseDetailsModal} handleCloseRegisterDetails={handleCloseRegisterDetails} setShowCloseDetailsModal={setShowCloseDetailsModal} />
        </Container>
    )
};
